...
export abstract class BaseChatModel {
    ...
    async generate(messages: ChatMessage[], stop?: string[]): Promise<ChatResult> {
        return this._generate(messages, stop);
    }

    abstract async _generate(messages: ChatMessage[], stop?: string[]): Promise<ChatResult>;
...
export abstract class SimpleChatModel extends BaseChatModel {
    ...
    abstract async _call(messages: ChatMessage[], stop?: string[]): Promise<string>;

    async _generate(messages: ChatMessage[], stop?: string[]): Promise<ChatResult> {
