...
import {
  ...
  MessageType, SystemChatMessage,
} from "../schema/index.js";
import {
  GoogleVertexAiBaseLLMInput,
  GoogleVertexAiConnection,
  GoogleVertexAiBasePrediction, GoogleVertexAiModelParams, GoogleVertexAiLLMResponse
...
export interface GoogleVertexAiChatCallOptions extends BaseLanguageModelCallOptions {
  context?: string;
  ...
  implements GoogleVertexAiChatInput
{
  ...
  async _generate(
    ...
    const instance: GoogleVertexAiChatInstance = {
      context: fromMessages.context ?? fromOptions.context ?? this.context ?? "",
      examples: fromOptions.examples ?? this.convertExamples(this.examples) ?? [],
  ...
  convertOptions(options: GoogleVertexAiChatCallOptions): GoogleVertexAiChatInstance {
    const examples = options?.examples ? this.convertExamples( options.examples ) : [];
  ...
  convertExamples(examples: ChatExample[] | undefined): GoogleVertexAiChatExample[] {
    return examples?.map((example) => this.convertExample(example)) ?? [];
  }

  convertResult(result: GoogleVertexAiLLMResponse<GoogleVertexAiChatPrediction>): ChatResult {
  ...
  convertPredictions( predictions: GoogleVertexAiChatPrediction[] ): ChatGeneration[] {
  ...
  convertPredictionMessage( message: GoogleVertexAiChatMessage ): BaseChatMessage {
