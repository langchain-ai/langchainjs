...
export class GmailSendMessage extends GmailBaseTool {
    ...
    private createEmailMessage({ message, to, subject, cc, bcc }: SendMessageParams): string {
    ...
    protected async _call({ message, to, subject, cc, bcc }: SendMessageParams): Promise<string> {
        return this.sendMessage({ message, to, subject, cc, bcc });
    }

    async sendMessage({ message, to, subject, cc, bcc }: SendMessageParams): Promise<string> {
        const auth = await this.getAuth();
        const gmail = google.gmail({ version: "v1", auth });

        const rawMessage = this.createEmailMessage({ message, to, subject, cc, bcc });
