  ...
  async _call(input: string) {
    ...
    if (answer_box) {
      if (answer_box.result) {
        ...
        Object.keys(answer_box).filter(
          (k) => !Array.isArray(answer_box[k]) && 
    ...
    if (res.top_sights?.sights) {
      const sights: Array<{[key: string]: string}> = res.top_sights.sights.map((s: {[key: string]: string}) => ({
        title: s.title,
    ...
    if (res.images_results && res.images_results[0]?.thumbnail) {
      return res.images_results.map((ir: { thumbnail: string }) => ir.thumbnail).slice(0, 10).toString()
    }
    ...
    if (res.knowledge_graph) {
      ...
      Object.keys(res.knowledge_graph).filter(
        (k) => typeof res.knowledge_graph[k] === "string" && 
          k !== "title" && 
          k !== "description" && 
          !k.endsWith("_stick") && 
          !k.endsWith("_link") && 
