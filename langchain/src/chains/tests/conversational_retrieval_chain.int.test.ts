...
test("Test ConversationalRetrievalQAChain from LLM with a chat model, memory, and keeping the chat history context", async () => {
  const model = new ChatOpenAI({
    modelName: "gpt-3.5-turbo",
    temperature: 0,
  });
  ...
  const chain = ConversationalRetrievalQAChain.fromLLM(
    ...
    vectorStore.asRetriever(),
    {
      ...
      questionGeneratorChainOptions: {
        extractConversationContext: true,
      },
    }
  );
  ...
  expect(res2).toEqual(
    expect.objectContaining({
      text: expect.stringContaining("28"),
    })
  );
});

test("Test ConversationalRetrievalQAChain from LLM with a chat model, memory, and without keeping the chat history context", async () => {
  const model = new ChatOpenAI({
    modelName: "gpt-3.5-turbo",
    temperature: 0,
  });
  ...
  const chain = ConversationalRetrievalQAChain.fromLLM(
    ...
    vectorStore.asRetriever(),
    {
      ...
      questionGeneratorChainOptions: {
        extractConversationContext: false,
      },
    }
  );
  ...
  expect(res2).toEqual(
    expect.objectContaining({
      text: expect.not.stringMatching("28"),
    })
  );
});
