...
import { DSVLoader } from "../fs/dsv.js";
import { Document } from "../../document.js";

test("Test DSV loader from file with column arg", async () => {
  const filePath = path.resolve(
    path.dirname(url.fileURLToPath(import.meta.url)),
    "./example_data/Star_Wars_The_Clone_Wars_S06E07_Crisis_at_the_Heart_pipe_delimated.csv"
  );
...
test("Test DSV loader without column arg but with delimiter only", async () => {
  const filePath = path.resolve(
    path.dirname(url.fileURLToPath(import.meta.url)),
    "./example_data/Star_Wars_The_Clone_Wars_S06E07_Crisis_at_the_Heart_pipe_delimated.csv"
  );
