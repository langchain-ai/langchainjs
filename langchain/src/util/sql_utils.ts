...
export const verifyListTablesExistInDatabase = (tablesFromDatabase: Array<SqlTable>, listTables: Array<string>, errorPrefixMsg: string): void => {
    const onlyTableNames: Array<string> = tablesFromDatabase.map((table: SqlTable) => table.tableName);
...
export const verifyIncludeTablesExistInDatabase = (tablesFromDatabase: Array<SqlTable>, includeTables: Array<string>): void => {
    verifyListTablesExistInDatabase(tablesFromDatabase, includeTables, 'Wrong include table name:');
};

export const verifyIgnoreTablesExistInDatabase = (tablesFromDatabase: Array<SqlTable>, ignoreTables: Array<string>): void => {
    verifyListTablesExistInDatabase(tablesFromDatabase, ignoreTables, 'Wrong ignore table name:');
};


const formatToSqlTable = (rawResultsTableAndColumn: Array<RawResultTableAndColumn>): Array<SqlTable> => {
...
export const getTableAndColumnsName = async (appDataSource: DataSource): Promise<Array<SqlTable>> => {
    let sql;
...
// TODO: add tests
export const generateTableInfoFromTables = async (tables:Array<SqlTable>|undefined, appDataSource: DataSource, nbSampleRow: number): Promise<string> => {
    if (!tables) {
        return '';
    }
    ...
    for (const currentTable of tables) {
        ...
        globalString = globalString.concat(sqlCreateTableQuery + sqlSelectInfoQuery + columnNamesConcatString + sample);
    }
