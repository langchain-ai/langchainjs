...
test("Calculator tool, sum", async () => {
  ...
  const result = await calculator._call("1 + 1");
...
test("Calculator tool, division", async () => {
  const calculator = new Calculator();
  const result = await calculator._call("7 /2");
  expect(result).toBe("3.5");
});

test("Calculator tool, exponentiation", async () => {
  const calculator = new Calculator();
  const result = await calculator._call("2 ^ 8");
  expect(result).toBe("256");
});

test("Calculator tool, complicated expression", async () => {
  const calculator = new Calculator();
  const result = await calculator._call("((2 + 3) * 4) / 2");
