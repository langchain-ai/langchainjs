...
const updateConfig = () => {
  ...
  Object.entries(entrypoints).forEach(([key, value]) => {
    ...
    const modulePath = path.basename(withoutExt) === 'index' ? path.dirname(withoutExt) : withoutExt;
    const compiledPath = `./dist/${modulePath}`;
    fs.writeFileSync(`./${key}.js`, `module.exports = require('${compiledPath}')`);
    fs.writeFileSync(`./${key}.d.ts`, `export * from '${compiledPath}'`);
  });

  fs.writeFileSync('./.gitignore', Object.keys(entrypoints).flatMap(key => [
    `${key}.js`,
    `${key}.d.ts`,
  ]).join("\n"));
}
