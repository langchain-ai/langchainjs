...
You should generally not export your new module from an `index.ts` file that contains many other exports. Instead, you should add a separate entrypoint for your integration in [`langchain/scripts/create-entrypoints.js`](langchain/scripts/create-entrypoints.js) within the `entrypoints` object:
...
You must also add your new entrypoint under `requiresOptionalDependency` in the [`create-entrypoints.js`](langchain/scripts/create-entrypoints.js) file to avoid breaking the build:
...
- [LLM providers](.github/contributing/integrations/LLMS.md) (e.g. OpenAI's GPT-3)
- Chat model providers (TODO) (e.g. Anthropic's Claude, OpenAI's GPT-4)
- [Memory](.github/contributing/integrations/MEMORY.md) (used to give an LLM or chat model context of past conversations, e.g. Motörhead)
- [Vector stores](.github/contributing/integrations/VECTOR_STORES.md) (e.g. Pinecone)
- [Persistent message stores](.github/contributing/integrations/LLMS.md) (used to persistently store and load raw chat histories, e.g. Redis)
- [Document loaders](.github/contributing/integrations/DOCUMENT_LOADERS.md) (used to load documents for later storage into vector stores, e.g. Apify)
- Embeddings (TODO) (e.g. Cohere)
- [Tools](.github/contributing/integrations/TOOLS.md) (used for agents, e.g. the SERP API tool)
