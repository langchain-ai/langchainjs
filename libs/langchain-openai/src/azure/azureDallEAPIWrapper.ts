 ...
 */
export type OpenAIImageModelId = "dall-e-2" | "dall-e-3";

/**
 * An interface for the Azure Dall-E API Wrapper.
 */
export interface AzureDallEAPIWrapperParams extends ToolParams {
  /**
 ...
 */
export class AzureDallEAPIWrapper extends Tool {
  static lc_name() {
  ...
  constructor(params: AzureDallEAPIWrapperParams) {
    // Shim for new base tool param name
    if (
      params.responseFormat !== undefined &&
      ["url", "b64_json"].includes(params.responseFormat)
    ) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      params.dallEResponseFormat = params.responseFormat as any;
      params.responseFormat = "content";
    }
    super(params);
