...
import { AzureSDKOpenAI } from "../azure/llms_sdk.js";
...
test("Test OpenAI with chat model returns OpenAIChat", async () => {
  const model = new AzureSDKOpenAI({ modelName: "gpt-3.5-turbo" });
  expect(model).toBeInstanceOf(AzureSDKChatOpenAI);
  const res = await model.call("Print hello world");
  console.log({ res });
...
test("Test OpenAI with instruct model returns OpenAI", async () => {
  const model = new AzureSDKOpenAI({ modelName: "gpt-3.5-turbo-instruct" });
  expect(model).toBeInstanceOf(AzureSDKOpenAI);
  const res = await model.call("Print hello world");
  console.log({ res });
  expect(typeof res).toBe("string");
});
