...
test("Test Azure OpenAI serialization from base path", async () => {
  ...
  expect(JSON.stringify(chat)).toEqual(
    `{"lc":1,"type":"constructor","id":["langchain","chat_models","azure_openai","AzureChatOpenAI"],"kwargs":{"openai_api_key":{"lc":1,"type":"secret","id":["OPENAI_API_KEY"]},"azure_endpoint":"https://langchain-ai.openai.azure.com/","deployment_name":"gpt-4o"}}`
  );
});
