...
test("It throws when there is no base llm provided", async () => {
  expect(() => {
    // @ts-ignore: testing error handling
    new ArcjetRedact({});
  }).toThrow(new Error("Arcjet Redact requires an LLM to be provided"));
});

test("It throws when entities are not all strings", async () => {
  const mockLLM = new MockLLM();
  const options = {
    llm: mockLLM,
    entities: ["email", true],
  ...
  expect(() => {
    // @ts-ignore: testing error handling
    new ArcjetRedact(options);
  }).toThrow(new Error("entities must all be strings"));
});

test("It throws when context window size isn't a number", async () => {
  const mockLLM = new MockLLM();
  const options = {
    llm: mockLLM,
    contextWindowSize: "hello-world"
  };

  expect(() => {
    // @ts-ignore: testing error handling
    new ArcjetRedact(options);
  }).toThrow(new Error("contextWindowSize must be a number"));
});

test("It throws when detect isn't a function", async () => {
  const mockLLM = new MockLLM();
  const options = {
    llm: mockLLM,
    detect: "hello-world",
  ...
  expect(() => {
    // @ts-ignore: testing error handling
    new ArcjetRedact(options);
  }).toThrow(new Error("detect must be a function"));
});

test("It throws when replace isn't a function", async () => {
  const mockLLM = new MockLLM();
  const options = {
    llm: mockLLM,
    replace: "hello-world",
  ...
  expect(() => {
    // @ts-ignore: testing error handling
    new ArcjetRedact(options);
  }).toThrow(new Error("replace must be a function"));
});
