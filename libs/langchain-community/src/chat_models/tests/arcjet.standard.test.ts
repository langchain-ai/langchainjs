...
class MockChatModel extends BaseChatModel {
  ...
  async _generate(messages: BaseMessage[], _options: this["ParsedCallOptions"], _runManager?: CallbackManagerForLLMRun | undefined): Promise<ChatResult> {
    return {
...
test("It passes messages through correctly", async () => {
  ...
  const output = await arcjetRedact.invoke(
    ["this is the input", "this is a second input"]
  );
...
test("It redacts built in entities across multiple messages and unredacts them in the response", async () => {
  const generationA = {
    message: new GenericMessage("Your email is <Redacted email #1> and your card number is <Redacted credit card number #2>"),
  ...
  const callback = (input: BaseMessage[]) => {
    ...
    expect(input[1].content).toEqual("my card number is <Redacted credit card number #2>");
    return [generationA]
  }
  ...
  const output = await arcjetRedact.stream(
    ["my email address is test@example.com", "my card number is 4242424242424242"]
  );

  const first = await output.next();
  expect(first.value.content).toEqual("Your email is test@example.com and your card number is 4242424242424242");
});
...
test("it handles custom replace functions correctly", async () => {
  const generationA = {
    message: new GenericMessage("custom is <Redacted custom-entity #1> email is redacted@example.com"),
    text: "custom is <Redacted custom-entity #1> email is redacted@example.com"
  };

  const callback = (input: BaseMessage[]) => {
    expect(input[0].content).toEqual("custom <Redacted custom-entity #1> email redacted@example.com");
    return [generationA]
  }
  ...
  expect(output.content).toEqual("custom is my-custom-string-to-be-detected email is test@example.com");
});
