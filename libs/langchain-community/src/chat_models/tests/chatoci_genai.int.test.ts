...
const creationParameters = [[{
  compartmentId,
  onDemandModelId: process.env.OCI_GENAI_INTEGRATION_TESTS_COHERE_ON_DEMAND_MODEL_ID
}], [{
  compartmentId,
  onDemandModelId: process.env.OCI_GENAI_INTEGRATION_TESTS_GENERIC_ON_DEMAND_MODEL_ID
}]];
  ...
  await testEachChatModelType(
    async (ChatClassType: OciGenAiChatConstructor, creationParams: any[]) => {
      for (const params of creationParams) {
        ...
        const response = await chatClass.invoke("generate a single, very short mission statement for a pet insurance company");
