...
import { describe, test, beforeEach, afterAll, expect, beforeAll } from "@jest/globals";
...
describe("CouchbaseSearchVectorStore", () => {
  // Configuration
  const config = {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      if (err.code !== 605) { // 605 is bucket_exists error
    ...
    try {
      ...
      if (err.code !== 12016) { // 12016 is index_exists error
  ...
  beforeEach(async () => {
    ...
    try {
      ...
      store = await CouchbaseSearchVectorStore.initialize(embeddings, storeConfig);
    } catch (error) {
  ...
  describe("Document Operations", () => {
    test("should add documents with metadata", async () => {
      ...
      const documents = texts.map((text, i) => new Document({ pageContent: text, metadata: metadatas[i] }));
