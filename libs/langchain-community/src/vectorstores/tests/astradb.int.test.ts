...
describe('AstraDBVectorStore', () => { 
  ...
  test('addDocuments', async () => {
    ...
    const pageContent: string[] = [faker.lorem.sentence(5), faker.lorem.sentence(5)];
    const metadata = [{ foo: "bar" }, { foo: "baz" }];

    await store.addDocuments(
      pageContent.map((content, idx) => new Document({ pageContent: content, metadata: metadata[idx] }))
    );

    const results = await store.similaritySearch(pageContent[0], 1);

    expect(results).toEqual([new Document({ pageContent: pageContent[0], metadata: metadata[0] })]);
  });
  ...
  test('fromExistingIndex', async () => {
    ...
    const results = await store2.similaritySearch(
      "Apache Cassandra",
      1
    );
