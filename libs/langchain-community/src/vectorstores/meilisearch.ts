...
export class MeiliSearchVectorStore extends VectorStore {
  ...
  async createMeiliSearchIndex(): Promise<void> {
    ...
    if (!indexExists) {
      ...
      const update_result = await this.client
        .index(this.index_name)
        .updateEmbedders(this.embedders);
  ...
  async addDocuments(documents: DocumentInterface[]): Promise<void> {
    ...
    const add_result = await this.client
      .index(this.index_name)
      .addDocuments(docs);
  ...
  async deleteAllDocuments(): Promise<void> {
    const delete_result = await this.client
      .index(this.index_name)
      .deleteAllDocuments();
    await this.client.waitForTask(delete_result.taskUid);
    return;
  ...
  async enableVectorStore(): Promise<void> {
    const result = await this.client.httpRequest.patch(
  ...
  static async fromTexts(
    ...
    const meilisearch = await MeiliSearchVectorStore.fromDocuments(
      docs,
      embeddings,
      args
    );
