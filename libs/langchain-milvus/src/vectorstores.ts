...
export class MilvusVectorStore extends VectorStore {
  ...
  async similaritySearchVectorWithScore(
    ...
    searchResp.results.forEach(
      (result: { id: string; score: number; [key: string]: unknown }) => {
        const fields = {
          pageContent: "",
        ...
        Object.keys(result).forEach((key) => {
          if (key === this.textField) {
            fields.pageContent = result[key] as string;
          } else if (this.fields.includes(key) || key === this.primaryField) {
            if (typeof result[key] === "string") {
              const { isJson, obj } = checkJsonString(result[key] as string);
    ...
    desc.schema.fields.forEach(
      (field: {
        ...
        if (
          ...
          field.name === MILVUS_TEXT_FIELD_NAME
        ) {
