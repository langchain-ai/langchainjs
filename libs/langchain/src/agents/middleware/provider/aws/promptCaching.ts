 ...
 * @public
 */
export function bedrockPromptCaching(
      ...
      if (!isBedrockConverseModel) {
        ...
        if (unsupportedModelBehavior === "raise") {
          throw new PromptCachingMiddlewareError(
            `${baseMessage} (e.g., 'us.anthropic.claude-haiku-4-5-20251001-v1:0').`
          );
        } else if (unsupportedModelBehavior === "warn") {
          console.warn(
            `PromptCachingMiddleware: Skipping caching for ${modelName}. Consider switching to an AWS Bedrock Converse model for caching benefits.`
          );
        }
        return handler(request);
      }
