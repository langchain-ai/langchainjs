 ...
 * `VastbaseVectorStore` instance. It includes Vastbase connection options,
 * table name, filter, and verbosity level.
 */
export interface VastbaseVectorStoreArgs {
  vastbaseConnectionOptions?: PoolConfig;
  pool?: Pool;
  tableName: string;
...
/**
 * Vastbase vector store integration.
 *
 * Setup:
 ...
 * ```
 *
 * ## [Constructor args](https://api.js.langchain.com/classes/_langchain_community.vectorstores_vastbase.VastbaseVectorStore.html#constructor)
 *
 * <details open>
 ...
 *   VastbaseVectorStore,
 *   DistanceStrategy,
 * } from "@langchain/community/vectorstores/vastbase";
 *
 * // Or other embeddings
 ...
 * };
 *
 * const vectorStore = await VastbaseVectorStore.initialize(embeddings, config);
 * ```
 * </details>
 *
 * <br />
 ...
 * <br />
 */
export class VastbaseVectorStore extends VectorStore {
  ...
  constructor(
    ...
    config: VastbaseVectorStoreArgs
  ) {
    ...
    if (!config.vastbaseConnectionOptions && !config.pool) {
      throw new Error(
        "You must provide either a `vastbaseConnectionOptions` object or a `pool` instance."
      );
    }
    const pool = config.pool ?? new pg.Pool(config.vastbaseConnectionOptions);
    this.pool = pool;
  ...
  /**
   * Static method to create a new `VastbaseVectorStore` instance from a
   * connection. It creates a table if one does not exist, and calls
   * `connect` to return a new instance of `VastbaseVectorStore`.
   *
   * @param embeddings - Embeddings instance.
   * @param fields - `VastbaseVectorStoreArgs` instance
   ...
   * @returns A new instance of `VastbaseVectorStore`.
   */
  static async initialize(
    embeddings: EmbeddingsInterface,
    config: VastbaseVectorStoreArgs & { dimensions?: number }
  ): Promise<VastbaseVectorStore> {
    const { dimensions, ...rest } = config;
    const vastbaseVectorStore = new VastbaseVectorStore(embeddings, rest);

    await vastbaseVectorStore._initializeClient();
    await vastbaseVectorStore.ensureTableInDatabase(dimensions);
    if (vastbaseVectorStore.collectionTableName) {
   ...
   * exactly and metadata filter according to vastbase jsonb containment. Ids and filter
  ...
  async similaritySearchVectorWithScore(
    ...
    return this.searchVastbase(query, k, filter, false);
  }
  ...
  /**
   * Static method to create a new `VastbaseVectorStore` instance from an
   ...
   * @param embeddings - Embeddings instance.
   * @param dbConfig - `VastbaseVectorStoreArgs` instance.
   * @returns Promise that resolves with a new instance of `VastbaseVectorStore`.
   */
  static async fromTexts(
    texts: string[],
    metadatas: object[] | object,
    embeddings: EmbeddingsInterface,
    dbConfig: VastbaseVectorStoreArgs & { dimensions?: number }
  ): Promise<VastbaseVectorStore> {
    const docs = [];
    ...
    return VastbaseVectorStore.fromDocuments(docs, embeddings, dbConfig);
  }
  ...
  static async fromDocuments(
    ...
    const instance = await VastbaseVectorStore.initialize(embeddings, dbConfig);
  ...
  async createHnswIndex(config: {
    ...
    let idxDistanceFunction =
      config?.distanceFunction || "floatvector_cosine_ops";
    const prefix = config?.namespace ? `${config.namespace}_` : "";

    switch (this.distanceStrategy) {
      case "cosine":
        idxDistanceFunction = "floatvector_cosine_ops";
        break;
      case "innerProduct":
        idxDistanceFunction = "floatvector_ip_ops";
        break;
      case "euclidean":
        idxDistanceFunction = "floatvector_l2_ops";
        break;
      default:
        throw new Error(`Unknown distance strategy: ${this.distanceStrategy}`);
    }
  ...
  async maxMarginalRelevanceSearch(
    ...
    const docs = await this.searchVastbase(
      queryEmbedding,
      fetchK,
      filter,
      true
