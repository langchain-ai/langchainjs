...
import { prepareAndParseToolCall } from "./experimental/utils/tool_calling.js";
  ...
  >({
    ...
    const llm = RunnableLambda.from(
      async (input: BaseLanguageModelInput, config?: RunnableConfig) => {
        const result = await prepareAndParseToolCall({
          messages:
            BaseChatModel._convertInputToPromptValue(input).toChatMessages(),
          options: {
            tools,
            tool_choice: {
              type: "function",
