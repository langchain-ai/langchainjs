...
import PostgresEngine, { PostgresEngineArgs, Column, VectorStoreTableArgs } from "../engine.js";
...
describe("PostgresEngine Instance creation", () => {
  ...
  test('should throw an error if the URL passed to from_engine_args does not have the driver', async () => {
    ...
    await expect(createInstance).rejects.toThrow("Driver must be type 'postgresql+asyncpg'");
  });
