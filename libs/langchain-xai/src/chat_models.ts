...
type XAIRoleEnum = "system" | "assistant" | "user" | "function" | "tool";

export function messageToGroqRole(message: BaseMessage): XAIRoleEnum {
 ...
 * <br />
 */
export class ChatXAI extends BaseChatModel<
  ChatXAICallOptions,
  AIMessageChunk
> {
  ...
  constructor(fields?: ChatXAIInput) {
    super(fields ?? {});

    const apiKey = fields?.apiKey || getEnvironmentVariable("XAI_API_KEY");
    if (!apiKey) {
      throw new Error(
        `Groq API key not found. Please set the XAI_API_KEY environment variable or provide the key into "apiKey"`
      );
    }
  ...
  >(
    ...
    if (!includeRaw) {
      return llm.pipe(outputParser).withConfig({
        runName: "ChatXAIStructuredOutput",
      });
    }
