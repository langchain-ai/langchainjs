...
const generateImportMap = ({
  ...
  const dynamicImportContent = dynamicImportMapEntries.map(({modules, alias, path}) => {
    const exportAlias = alias.join("__");
    return [
      `  const ${exportAlias}: Record<string, any> = {};`,
      `  try {`,
      `    const dynamicImports = await import("${path}");`,
      `    ${modules.map((module) => `${exportAlias}.${module} = dynamicImports.${module}`)};`,
      `  } catch (e) {}`
    ].join("\n");
  }).join("\n\n");
  ...
  const dynamicContent = [
    ...
    `  return {`,
    `    ${dynamicImportMapEntries.map(({alias}) => alias.join("__")).join(",\n    ")}`,
    `  };`,
    `}\n`
  ].join("\n");

  fs.writeFileSync(
    `../${pkg}/src/load/dynamic_import_map.ts`,
    `// Auto-generated by \`scripts/create-entrypoints.js\`. Do not edit manually.\n\n${dynamicContent}`
  );
};
