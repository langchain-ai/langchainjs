...
export function mapGenerateContentResultToChatResult(
  ...
  if (
    ...
    content = parts
      .map((p): MessageContentComplex | null => {
      ...
      .filter((block): block is MessageContentComplex => block !== null);
  } else {
    ...
    content = streamParts
      .map((p): MessageContentComplex | null => {
        if (p.thought && "text" in p && p.text) {
          ...
          // Tool calls are surfaced via `tool_call_chunks` instead; keeping
          // them out of `content` would otherwise duplicate them and leak a
          // Gemini-native block type into providers that reject it (e.g. OpenAI).
          return null;
        } else if ("functionResponse" in p) {
          return {
            type: "functionResponse",
