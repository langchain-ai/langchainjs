...
const videoFileExists = fs.existsSync(pathToVideoFile);
...
// Skip entire suite if video file is not downloaded
describe.skipIf(!videoFileExists)("context caching", () => {
  beforeAll(
    async () => {
      ...
      const fileCache = new GoogleAIFileManager(
        process.env.GOOGLE_API_KEY || ""
      );
