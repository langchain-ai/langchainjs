...
import { uri, waitForIndexToBeQueryable, waitForDocumentsIndexed } from "./utils.js";
...
describe.skipIf(skipAutoEmbedTests)("Auto Embedding Tests with data", () => {
  ...
  test("MongoDBStore sets client metadata", () => {
    ...
    new MongoDBAtlasVectorSearch(defaultEmbeddingModel, {
      collection,
    });
  ...
  test("MongoDBAtlasVectorSearch with external ids", async () => {
    const vectorStore = new MongoDBAtlasVectorSearch(defaultEmbeddingModel, { collection });
  ...
  test("MongoDBAtlasVectorSearch upsert", async () => {
    const vectorStore = new MongoDBAtlasVectorSearch(defaultEmbeddingModel, {
      collection,
    });
  ...
  describe("addDocuments method", () => {
    ...
    beforeEach(async () => {
      vectorStore = new MongoDBAtlasVectorSearch(defaultEmbeddingModel, {
        collection,
      });
    });
    test("correctly embeds and stores documents", async () => {
      ...
      const results = await collection
        .find(
          {},
          { projection: { _id: 0, text: 1 }, sort: { text: 1 } }
        )
        .toArray();
...
describe("Auto Embedding Tests without data", () => {
  ...
  test("addVectors method throws error when auto embedding is enabled", async () => {
    ...
    ).rejects.toThrow("Cannot add vectors directly when using auto-embedding mode.");
  });
  ...
  test("similaritySearchVectorWithScore method throws error when auto embedding is enabled", async () => {
    ...
    await expect(vectorStore.similaritySearchVectorWithScore([], 2))
      .rejects
      .toThrow("Cannot perform similarity search with vectors directly when using auto-embedding mode.");
  });
});
