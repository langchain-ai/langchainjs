...
export function removeLuceneChars(text: string | null) {
...
export function isVersionLessThan(v1: number[], v2: number[]): boolean {
...
function handleFieldFilter(
  ...
  if (COMPARISONS_TO_NATIVE_OPERATORS.has(operator)) {
    ...
    if (operator === "$in") {
      ...
      const queryParam = {
        [`param_${paramNumber}`]: filterValue.slice(0, -1),
      };
      return [querySnippet, queryParam];
