...
describe("convertMessagesToGeminiContents", () => {
  ...
  test("does not duplicate functionCall parts when content contains functionCall blocks (legacy path)", () => {
    const messages = [
      new HumanMessage("use two tools"),
      new AIMessage({
        content: [
          {
            type: "functionCall",
    ...
    // Should have exactly 2 functionCall parts, not 4 (duplicated)
    expect(functionCallParts).toHaveLength(2);
    expect(
      (functionCallParts[0] as Gemini.Part.FunctionCall).functionCall!.name
    ).toBe("list_directory");
    expect(
      (functionCallParts[1] as Gemini.Part.FunctionCall).functionCall!.name
    ).toBe("read_file");
  });

  test("preserves thoughtSignature on functionCall parts (legacy path)", () => {
    const messages = [
    ...
    expect(functionCallPart.thoughtSignature).toBe("abc123signature");
  });

  test("preserves thoughtSignature on functionCall parts (standard path)", () => {
    const aiMsg = new AIMessage({
      content: [],
      tool_calls: [
        {
    ...
    aiMsg.response_metadata = { output_version: "v1" };
    const messages = [new HumanMessage("use a tool"), aiMsg];
