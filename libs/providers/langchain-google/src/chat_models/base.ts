...
import {
  convertGenerateContentResponseToAIMessage,
...
> extends BaseChatModel<CallOptions, AIMessageChunk> {
  ...
  async _generate(
    ...
    const message = convertGenerateContentResponseToAIMessage(candidate);
      ...
      outputParser = RunnableLambda.from<BaseMessage, RunOutput>(
        async (input: BaseMessage): Promise<RunOutput> => {
          ...
          if (isInteropZodSchema(schema)) {
            ...
            return (await zodParser.parse(content)) as RunOutput;
          } else {
