...
describe("Chat unit tests", () => {
  describe("Positive tests for default usage", () => {
    ...
    test("Tool conversion handles both Zod schemas and JSON schemas", () => {
      ...
      const chatModel = new ChatWatsonx({ ...testProps, ...fakeAuthProp });
      ...
      expect(() => {
        const modelWithTools = chatModel.bindTools([zodTool, jsonSchemaTool]);
      ...
      expect(() => {
        const modelWithMcpTool = chatModel.bindTools([mcpLikeTool]);
