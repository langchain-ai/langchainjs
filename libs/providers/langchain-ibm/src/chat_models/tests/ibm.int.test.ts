  ...
  ({ params }) => {
        ...
        const service = new ChatWatsonx({
          ...
          callbackManager: CallbackManager.fromHandlers({
            ...
            async handleLLMNewToken(token: string, idx: NewTokenIndices) {
              ...
              if (token) tokenUsed += 1;
            },
          }),
        });
        const message = new HumanMessage("Can you descirbe what an LLM is?");
        const res = await service.generate([[message], [message]]);
    ...
    describe("Test tool usage", () => {
      ...
      test("Passing tool to chat model", async () => {
        ...
        expect(typeof res.tool_calls?.[0].args?.number1).toBeDefined();
        expect(typeof res.tool_calls?.[0].args?.number2).toBeDefined();
      ...
      test("Passing tool to chat model extended", async () => {
        ...
        for (const tool_call of res.tool_calls) {
          ...
          expect(typeof tool_call.args?.number1).toBeDefined();
          expect(typeof tool_call.args?.number2).toBeDefined();
        }
      });
      ...
      test("Passing tool to chat model - multiple tool_calls in one prompt", async () => {
        const service = new ChatWatsonx({
          version,
