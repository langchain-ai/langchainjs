...
describe("LLM unit tests", () => {
  describe("Positive tests with default mode", () => {
    ...
    test("Test basic properties after init with idOrName", async () => {
      const testProps = {
        version: "2024-05-31",
        serviceUrl,
        idOrName: "testString",
  ...
  describe("AbortSignal parameter passing", () => {
    ...
    const instance = new WatsonxLLM({ ...testProps, ...fakeAuthProp });
    test("Signal passed to generateText() with projectId", async () => {
      const mockResponse = {
        results: [
          {
            generated_text: "",
    ...
    test("Signal passed to generateTextStream() with projectId", async () => {
