 ...
 *
 * Setup:
 * Install `@langchain/pgvector` and `pg`.
 *
 * If you wish to generate ids, you should also install the `uuid` package.
 *
 * ```bash
 * npm install @langchain/pgvector pg uuid
 * ```
 *
 * ## [Constructor args](https://api.js.langchain.com/classes/_langchain_pgvector.PGVectorStore.html#constructor)
 *
 * <details open>
...
export class PGVectorStore extends VectorStore {
  ...
  private convertDistanceToScore(distance: number): number {
    return this.scoreNormalization === "distance"
      ? distance
      : this.convertDistanceToSimilarity(distance);
  }
