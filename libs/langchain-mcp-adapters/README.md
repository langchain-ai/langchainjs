...
Supply an application-owned `authProvider`. Use `OAuthClientProvider` when the
official SDK should handle discovery, registration, exchange and refresh, or
`AuthProvider` when your token broker owns authorization. Both types are exported
by the adapter. Your application owns credential storage and account binding.

For an OAuth redirect, restore the initiating user's provider and call
`finishAuth(serverName, callbackParams, expectedState)`. See the
[OAuth recipe](https://github.com/langchain-ai/langchainjs/blob/main/libs/langchain-mcp-adapters/examples/oauth.ts).
URL elicitation concerns a tool's external action; it is separate from connection OAuth.
