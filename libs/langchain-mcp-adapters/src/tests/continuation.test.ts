...
it.each(["state-only", "questions", "abort", "transport"])(
  "rejects an incomplete direct response without polling: %s",
  async (scenario) => {
    ...
    const invocation = withMCPInterrupts(
      async (continuation) => {
        calls += 1;
        // The direct boundary never continues a call, so it never supplies one.
        ...
        throw new PendingMCPInput(
          {
            kind: "input_required",
            inputRequests:
              scenario === "questions" ? { confirm: question } : {},
    ...
    // A state-only response is rejected, not retried on a timer.
    expect(calls).toBe(1);
  }
);
