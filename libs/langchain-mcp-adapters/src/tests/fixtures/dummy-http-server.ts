...
import { McpServer, ResourceTemplate, isInitializeRequest } from "@modelcontextprotocol/server";
import { z } from "zod";
...
export function createDummyHttpServer(
  ...
  function createMcpServerInstance(): McpServer {
    ...
    server.registerTool("test_tool", { description: "A test tool that echoes input and request metadata", inputSchema: z.object({ input: z.string() }) }, async ({ input }, ctx) => {
                  // Logging message
    ...
    // oxlint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-ignore - this may raise "Type instantiation is excessively deep and possibly infinite.ts(2589)"
    server.registerTool("sleep_tool", { description: "A test tool that sleeps for the given number of milliseconds before returning", inputSchema: z.object({ sleepMsec: z.number().int().positive() }) }, async ({ sleepMsec }) => {
    ...
    if (options.testHeaders) {
      server.registerTool("check_headers", { description: "Check if specific headers were received", inputSchema: z.object({ headerName: z.string() }) }, async ({ headerName }, ctx) => {
    ...
    server.registerTool("audio_tool", { description: "A tool that returns a dummy audio content.", inputSchema: z.object({
    ...
    server.registerTool("image_tool", { description: "A tool that returns a dummy image and text content.", inputSchema: z.object({
    ...
    server.registerTool("resource_tool", { description: "A tool that returns a dummy resource and text content.", inputSchema: z.object({
    ...
    server.registerTool("structured_tool", { description: "A tool that returns structuredContent and _meta", inputSchema: z.object({
