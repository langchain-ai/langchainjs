...
MCP tools return text, image, audio and resource blocks. The adapter converts
model-visible blocks into standard LangChain content. Use `outputHandling` to
send each content type to the model or retain it in `ToolMessage.artifact` for
your application, such as a dataframe used by another tool.
...
Alternatively, supply an SDK `AuthProvider` with `token()` and optional
`onUnauthorized()` methods when an application broker owns authorization.
The transport requests a token and delegates unauthorized handling to that
provider. This route does not run the SDK's OAuth registration or browser flow.
See the [OAuth recipe](../examples/oauth.ts) for both provider contracts.

### Complete an OAuth callback

The application owns the redirect endpoint and a one-time state value bound to
the initiating user. An `OAuthClientProvider` must persist discovery state,
the Proof Key for Code Exchange (PKCE) verifier, and issuer-scoped credentials.
Reconstruct that provider before completing a callback on another worker.

Pass the full redirect parameters, including `iss`, to
`adapter.finishAuth(serverName, callbackParams, expectedState)`. Obtain
`expectedState` from the application's authorization attempt, not the redirect
query. The adapter checks state, delegates exchange and issuer validation to
the SDK, then discards the completed attempt's connection/catalog. Applications
own one-time attempt consumption and expiry. `finishAuth()` rejects token-only
providers because their authorization is managed externally.

### Registration and discovery storage

The SDK selects Client ID Metadata Documents (CIMD), Dynamic Client Registration
(DCR) compatibility, or existing registration according to the authorization
server's capabilities. This decision is independent of MCP modern/legacy mode.
The SDK derives DCR `application_type` from redirect URIs; applications can
override it in `clientMetadata`.

Providers own discovery freshness. Preserve the discovery and PKCE state of an
in-flight attempt for its callback. When rediscovery selects another issuer,
the SDK rejects credentials registered with the previous issuer. Keep each
provider bound to one account and authorization server. Tool URL elicitation
is separate from OAuth authorization for the MCP connection.
...
expiration, and pagination; the adapter reuses adapted tools while the cached descriptors
