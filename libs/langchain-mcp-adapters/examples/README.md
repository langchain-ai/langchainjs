...
`oauth.ts` accepts application-owned SDK provider implementations. It shows how to
complete a saved OAuth attempt with the full callback query and how to use a token
broker. Supply your application's redirect handler and credential storage to use
this recipe; it does not run a standalone authorization server.
