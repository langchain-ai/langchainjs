    ...
    .map((msg) => {
      if (msg._getType() === "ai") {
        ...
        if (castMsg.tool_calls && castMsg.tool_calls.length) {
          // We should check the tool_calls first, as Claude sometimes includes them along with text content in its responses.
          const content: ContentBlock[] = [];
          // It looks like we don't need to reference the previous text generated by the LLM in this call.
          // if (typeof castMsg.content === "string" && castMsg.content !== "") {
          //   content.push({ text: castMsg.content });
          // }
          castMsg.tool_calls.forEach((tc) => (
            content.push({
              toolUse: {
                toolUseId: tc.id,
                name: tc.name,
        ...
        } else if (typeof castMsg.content === "string" && castMsg.content !== "") {
          return {
            role: "assistant",
            content: [
              {
          ...
          if (Array.isArray(castMsg.content)) {
...
export function handleConverseStreamContentBlockDelta(
  ...
  if (contentBlockDelta.delta.text !== undefined && typeof contentBlockDelta.delta.text === `string` ) {
