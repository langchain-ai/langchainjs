...
test("multiturn streaming", async () => {
  ...
  const modelWithToolsForced = model.bindTools([weatherTool], {
  ...
  const stream = await modelWithToolsForced.stream(messages);
...
test("multiturn invoking", async () => {
  ...
  const result = await modelWithToolsForced.invoke(messages);
