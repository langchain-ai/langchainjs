...
/**
 * Expands a type to show its full structure, useful for debugging complex types.
 *
 * @template T - The type to expand
 * @returns The expanded type with all properties visible
 */
export type $Expand<T> = T extends (...args: unknown[]) => unknown
  ? T
  : T extends object
  ? T extends ContentBlock
    ? T
    : {
