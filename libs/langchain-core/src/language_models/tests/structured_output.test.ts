...
describe("createFunctionCallingParser", () => {
  it("returns JsonOutputKeyToolsParser with zodSchema for a Zod v3 schema", () => {
    ...
    expect((parser as unknown as JsonOutputKeyToolsParser).keyName).toBe("extract");
    expect((parser as unknown as JsonOutputKeyToolsParser).zodSchema).toBeDefined();
  });
  ...
  it("uses the provided keyName", () => {
    ...
    expect((parser as unknown as JsonOutputKeyToolsParser).keyName).toBe("MyFunction");
  });
