...
Configure validated form and URL elicitation callbacks on individual legacy servers. Configure `logLevel` and `maxElicitationRounds` on servers that may negotiate modern, which is any server not pinned to `mode: "legacy"`. Modern catalog subscriptions keep tool caches current even without an application observer; subscription failures reject the connection. `setLoggingLevel()` remains legacy-only.
