...
as well as an `OAuthClientProvider`. `MCPAdapter#finishAuth(serverName,
callbackParams, { expectedState })` completes an OAuth redirect with the SDK's
issuer (RFC 9207) check and keeps no state between the redirect and the
callback. Provider auth failures are `MCPClientError`s whose `cause` is the
SDK's `UnauthorizedError`, now exported, and servers that failed on
