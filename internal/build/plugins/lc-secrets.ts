    ...
    async buildStart(this: PluginContext) {
      ...
      try {
        ...
        if (secrets.length > 0) {
          ...
          try {
            await generateSecretMap(packagePath, secrets, opts.outputPath);
 ...
 */
async function generateSecretMap(
  packagePath: string,
  secrets: SecretInfo[],
  outputPath: string
): Promise<void> {
  ...
  const rawContent = `// Auto-generated by lc-secrets plugin. Do not edit manually.
...
`;
  ...
  const content = await formatWithPrettier(rawContent, secretMapPath);
