...
/**
 * @param {Array<{ entrypoint: string, exportedSymbols: Array<{ symbol: string kind: SyntaxKind }> }>} entrypoints
 * @returns {Array<{ entrypoint: string, exportedSymbols: Array<{ symbol: string kind: SyntaxKind }> }>}
 */
function removeLoad(entrypoints) {
  return entrypoints.flatMap((entrypoint) => {
    ...
    const withoutLoadOrIndex = entrypoint.exportedSymbols.filter((item) => {
      if (newEntrypoint === "load") {
        if (item.symbol === "load" || item.symbol === "OptionalImportMap" || item.symbol === "optionalImportEntrypoints")
        return false;
      }
      return true;
    });
...
async function main() {
  ...
  /** @type {Array<{ entrypoint: string, exportedSymbols: Array<{ symbol: string kind: SyntaxKind }> }>} */
  const langchainCorePackageEntrypoints = removeLoad(getEntrypointsFromFile(path.join("langchain-core", "scripts", "create-entrypoints.js"), project));

  /** @type {Array<{ entrypoint: string, exportedSymbols: Array<{ symbol: string kind: SyntaxKind }> }>} */
  const langchainCommunityPackageEntrypoints = removeLoad(getEntrypointsFromFile(path.join("libs", "langchain-community", "scripts", "create-entrypoints.js"), project));

  /** @type {Array<{ entrypoint: string, exportedSymbols: Array<{ symbol: string kind: SyntaxKind }> }>} */
  const langchainOpenAIPackageEntrypoints = removeLoad(getEntrypointsFromFile(path.join("libs", "langchain-openai", "scripts", "create-entrypoints.js"), project));

  /** @type {Array<{ entrypoint: string, exportedSymbols: Array<{ symbol: string kind: SyntaxKind }> }>} */
  const langchainAnthropicPackageEntrypoints = removeLoad(getEntrypointsFromFile(path.join("libs", "langchain-anthropic", "scripts", "create-entrypoints.js"), project));


  const allLangChainFiles = glob.sync("./examples/src/**/*.ts").map((filePath) => path.resolve(filePath));
  ...
  for await (const lcFilePath of allLangChainFiles) {
    ...
    imports.flatMap((importItem) => {
      ...
      namedImports.forEach((namedImport) => {
        ...
        if (!namedImportKind) {
          console.log("no kind found", namedImportText)
          return;
        }
  ...
  console.log("Total missing", iter);
}
