...
1. Data processing for the intermediate steps (`agent_scratchpad`). These need to represented in a way that the language model can recognize them. This should be pretty tightly coupled to the instructions in the prompt. For this reason, we use the built in util `formatXml` to format the steps as XML.

2. The prompt itself. This is the default XML agent prompt, which includes variables for the tool list and user question. It also contains examples of inputs and outputs for the agent to learn from.
...
import { AIMessagePromptTemplate, ChatPromptTemplate } from "langchain/prompts";
...
const runnableAgent = RunnableSequence.from([
  {
    ...
    agent_scratchpad: (i: InputValues) => formatXml(i.steps),
  },
  prompt,
  model,
  outputParser,
