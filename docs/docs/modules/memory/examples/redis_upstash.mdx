...
Because upstash redis works via a REST API, you can use this in Edge and Serverless environments.
...
You will also need an Upstash Account and a Redis database to connect to. See instructions on [Upstash Docs](https://docs.upstash.com/redis/howto/connectclient#upstash-redis) on how to create a HTTP client.
...
import Example from "@examples/memory/redis_upstash.ts";
...
import AdvancedExample from "@examples/memory/redis_upstash_advanced.ts";
