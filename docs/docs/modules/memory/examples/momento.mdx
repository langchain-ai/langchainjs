...
For distributed, serverless persistence across chat sessions, you swap in a [Momento](https://gomomento.com/)-backed chat message history.
...
To distinguish one chat history session from another, we need a unique ID, the `sessionId`. You may also provide an optional `sessionTtl` to make sessions expire after a given number of seconds.
