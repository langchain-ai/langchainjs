...
const memory = new ConversationTokenBufferMemory({ llm: model, maxTokenLimit: 10 });
...
```typescript
const memory = new ConversationTokenBufferMemory({ llm: model, maxTokenLimit: 10, returnMessages: true });
```
