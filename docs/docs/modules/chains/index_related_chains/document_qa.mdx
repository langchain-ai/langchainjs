...
LangChain provides an assortment of chains specifically tailored for dealing with unstructured text data: `StuffDocumentsChain`, `MapReduceDocumentsChain`, and `RefineDocumentsChain`. These chains form the basic building blocks for developing more complex chains capable of processing unstructured text data. They are designed to take both documents and a question as input, then utilize the language model to formulate a response based on the provided documents.

- `StuffDocumentsChain`: This chain is the most straightforward among the three. It simply injects all incoming documents into the prompt and returns the answer to the question, using all documents as context. It is suitable for QA tasks over a small number of documents.
...
- `RefineDocumentsChain`: This chain iterates over the documents one by one, updating an ongoing answer with each iteration. It uses the previous version of the answer and the next document as context. It is suitable for QA tasks over a large number of documents.

## Usage of `StuffDocumentsChain` and `MapReduceDocumentsChain`

<CodeBlock language="typescript">{QAExample}</CodeBlock>

## Usage of `RefineDocumentsChain`
