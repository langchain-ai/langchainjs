...
## Answering Meta Questions About the Conversation
...
By setting `extractConversationContext` to true within in the `questionGeneratorChainOptions` object, you can use an alternative preset prompt for the
question generation chain that also returns parts of the chat history relevant to the answer, allowing the QA chain to answer meta questions with the additional context.

Here's an example:

import ConvoRetrievalQAWithContext from "@examples/chains/conversation_qa_with_memory_vector_store_and_keeping_chat_history_context.ts";

<CodeBlock language="typescript">{ConvoRetrievalQAWithContext}</CodeBlock>
