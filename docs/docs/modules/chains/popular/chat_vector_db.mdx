...
- The `context` key is used to fetch relevant documents from the loaded context (in this case the State Of The Union speech). It performs a call to the `getRelevantDocuments` method on the retriever, passing in the user's question as the query. We then pass it to our `formatDocumentsContentToString` util which maps over all returned documents, joins them with newlines and returns a string.
