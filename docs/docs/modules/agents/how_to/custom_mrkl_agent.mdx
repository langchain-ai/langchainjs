...
Custom `Runnable`
The first way to create a custom agent is not to use an existing Agent class, but use a custom `Runnable`. This is the simplest way to create a custom Agent.
