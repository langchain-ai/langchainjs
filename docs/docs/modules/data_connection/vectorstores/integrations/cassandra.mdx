...
In the configuration document, two optional parameters are provided; their defaults are:
...
| `batchSize` | How many documents will be sent on a single request. A value > 1 is recommended only when loading into a single partition. |