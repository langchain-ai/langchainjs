...
**Note:** OpenAI embeddings are too large in dimensionality for Atlas coming in at 1536. However, the Cohere embeddings are perfectly usable with a dimensionality of 1024. (See [limitations](#limitations).)
...
By default the vector store expects an index name of `default` although the configuration options allow this name to be arbitrary.
