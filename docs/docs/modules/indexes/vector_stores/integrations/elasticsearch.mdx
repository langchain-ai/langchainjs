...
You'll also need to have an Elasticsearch instance running. You can use the [official Docker image](https://www.elastic.co/guide/en/elasticsearch/reference/current/docker.html) to get started. You can also use [Elastic Cloud](https://www.elastic.co/cloud/) the official cloud service provided by Elastic.
...
Here we reported an example that index 4 documents in Elasticsearch as vector store,
runs a vector search query and use an LLM to answer a question in natural language
using the vector store as knowledge.
