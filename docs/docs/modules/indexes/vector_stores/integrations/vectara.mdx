...
LangChain connects to Vectara via the `Vectara-ts-client` package, the official Typescript client for Vectara.
Vectara provides both a SOTA optimized vector store and its own embeddings, so you don't have to calculate embeddings directly within the chain.
