...
Langchain supports hybrid search with a Supabase Postgres database. The hybrid search combines the postgres `pgvector`extension and Full-Text Search to retrieve documents. 
...
import Example from "@examples/indexes/vector_stores/supabase_hybrid.ts";
