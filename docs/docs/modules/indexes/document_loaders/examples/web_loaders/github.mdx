...
If you are using Node and would like to only load documents under a certain path, you can pass in a `ignorePaths` array to the constructor.

This requires the [ignore npm package](https://www.npmjs.com/package/ignore) as a peer dependency. Install it like this:

```bash npm2yarn
npm install ignore
```

Then pass in an `ignorePaths` array into the constructor:
