...
You can also pass in the optional `returnPromptLayerID` string to get a promptLayerRequestID like below
```typescript
import { PromptLayerOpenAI } from "langchain/llms/openai";
...
PromptLayer is compatible with the OpenAIChat model as well. Here is an example of getting the PromptLayerOpenAIChat requestID:

```typescript
import { PromptLayerOpenAIChat } from "langchain/llms/openai-chat";
