...
Note, for some providers this won't cancel the underlying request as they expose that option. Where made available by the provider, LangChain will cancel the underlying request, otherwise it will cancel the processing of the response.
