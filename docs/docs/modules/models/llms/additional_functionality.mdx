...
You can cancel a request by passing a `signal` option when you instantiate the model. For example, for OpenAI:

<CodeBlock language="typescript">{CancellationExample}</CodeBlock>

Currently, the signal option is only supported for OpenAI models.
