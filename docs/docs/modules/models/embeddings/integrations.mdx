...
This Embeddings integration uses the Xenova Transformers.js lib to generate embeddings for a given text using by default the `Xenova/all-MiniLM-L6-v2` model. You can pass a different model name to the constructor to use a different model.
...
```typescript
import { XenovaTransformersEmbeddings } from "langchain/embeddings/xenovatransformers";
