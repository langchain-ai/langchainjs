...
Next, we'll construct our chain.
...
This is somewhat of a complex chain so let's break it down.
...
- In the next block we capture the output from the previous block and pipe it through to the next prompt. We also preform a retrieval call to get the relevant documents for the question and any chat history which might be present.
- Finally, we pipe the output from the previous block through to the model and output parser. This will return the final answer as a string to the user.
