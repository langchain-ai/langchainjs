...
This model class is designed to work with normal `PromptTemplate`'s. An example of that is below:
...
```typescript
const prompt = PromptTemplate.fromTemplate("Human: Tell me a joke about {topic}");
const promptValue = prompt.formatPrompt({ topic: "bears" });
model.convertPrompt(promptValue)
```
