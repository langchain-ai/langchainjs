...
TensorFlow.js for embeddings, and FAISS for retrieval. We'll also install `cheerio` for scraping, though you can use any loader.

```bash npm2yarn
npm install @tensorflow/tfjs-core @tensorflow/tfjs-converter @tensorflow-models/universal-sentence-encoder @tensorflow/tfjs-node
npm install faiss-node
npm install cheerio
```

You'll also need to set up Ollama and run a local instance using [these instructions](https://github.com/jmorganca/ollama).
...
For an even simpler flow, use the preconfigured `RetrievalQA`.

This will use a QA default prompt and will retrieve from the vectorDB.

You can still pass in a custom prompt if desired, but it must include "context" and "query" input variables.

`type: "stuff"` (see [here](https://js.langchain.com/docs/modules/chains/document/stuff)) means that all the docs will be added (stuffed) into a prompt.
