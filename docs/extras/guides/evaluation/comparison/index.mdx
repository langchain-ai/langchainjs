...
To create a custom comparison evaluator, inherit from the `PairwiseStringEvaluator` or `LLMPairwiseStringEvaluator` class and overwrite the `__evaluateStringPairs` method. If you require asynchronous evaluation, also overwrite the `_a_evaluateStringPairs` method.
