...
# Additional Functionality: Agents

We offer a number of additional features for Agents. You should also look at the [LLM-specific features](/docs/modules/models/llms/additional_functionality) and [Chat Model-specific features](/docs/modules/models/chat/additional_functionality).

## Adding a timeout

By default, LangChain will wait indefinitely for a response from the model provider. If you want to add a timeout, you can pass a `timeout` option, in milliseconds, when you run the agent. For example:
...
You can subscribe to a number of events that are emitted by the Agent and the underlying tools, chains and models.
