...
2. Install the SDK for your environment.
   ...
   2.1. For backend environments install the Momento Node.js SDK:

   ```bash npm2yarn
   npm install @gomomento/sdk
   ```

   2.2. For browser or edge environments install the Momento Web SDK:
...
import TextsExample from "@examples/indexes/vector_stores/momento-vector-index/fromTexts.ts";
...
import DocsExample from "@examples/indexes/vector_stores/momento-vector-index/fromDocs.ts";

<CodeBlock language="typescript">{DocsExample}</CodeBlock>

### Search from an existing collection

import ExistingExample from "@examples/indexes/vector_stores/momento-vector-index/fromExisting.ts";
