...
OpenAI requires parameter schemas in the format below, where `parameters` must be in JSON Schema format.
Specifying the `function_call` parameter will force the model to return a response using the specified function.
If you have multiple schems you'd like the model to pick from
