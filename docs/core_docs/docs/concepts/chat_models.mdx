...
These models implement the [BaseLLM](https://api.js.langchain.com/classes/_langchain_core.language_models_llms.BaseLLM.html) interface and may be named with the "LLM" suffix (e.g., `OllamaLLM`, `AnthropicLLM`, `OpenAILLM`, etc.). Generally, users should not use these models.
:::
...
5. **withStructuredOutput**: A wrapper around the `invoke` method for models that natively support [structured output](/docs/concepts#structured_output).
