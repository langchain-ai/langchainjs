...
You can invoke the script by calling `pnpm release`. If new dependencies have been added to the integration package, install them first (i.e. run `pnpm`, then `pnpm release`).
...
To generate and view the documentation locally, change to the project root and run `pnpm` to ensure dependencies get installed
