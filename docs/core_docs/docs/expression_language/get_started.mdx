...
We first instantiated our model, vector store and output parser. Then we defined out prompt, which takes in two input variables:
...
Next we created a `setupAndRetriever` runnable. This has two components:
...
- `question` -> this uses a `RunnablePassthrough` which simply passes whatever the input was through, and in our case it returns it to the key in the object we defined.
