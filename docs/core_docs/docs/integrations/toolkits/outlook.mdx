...
Basically you only need the accessToken to use the Outlook Toolkit, but in expires in an hour.

If you only have a valid accessToken, the following two ways creates a Outlook tool instance.  
```typescript
...
```
But if you do not have a valid accessToken, the `AuthFlowREST` class obtains the accessToken for you. A temporary server will be opened on your `redirectUri` and you need to manually open the printed link to login on behalf of the user. For every `AuthFlowREST` instance, you only need to login once. 

Credentials you need to provide:`clientId`, `clientSecret`, `redirectUri`
