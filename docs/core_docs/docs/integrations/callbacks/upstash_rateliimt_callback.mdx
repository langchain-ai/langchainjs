...
In this guide, we will go over how to add rate limiting based on number of requests or the number of tokens using `UpstashRatelimitHandler`. This handler uses [ratelimit library of Upstash](https://github.com/upstash/ratelimit-js/), which utilizes [Upstash Redis](https://upstash.com/docs/redis/overall/getstarted).
...
```tsx
const response = ratelimit.limit()
if (response.success) {
...
Next, you will need to install Upstash Ratelimit:
...
import { UpstashRatelimitHandler, UpstashRatelimitError } from "@langchain/community/callbacks/handlers/upstash_ratelimit";
...
Not all LLMs in LangChain Js comply with this format however. If your llm returns the same values with differnet keys, you can use the parameters `llmOutputTokenUsageField`, `llmOutputTotalTokenField` and `llmOutputPromptTokenField` by passing them to the handler:
