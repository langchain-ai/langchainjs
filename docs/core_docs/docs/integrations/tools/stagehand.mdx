...
2. Set up your model provider credentials:
...
env: "LOCAL", // or "BROWSERBASE" for remote browser
});
...
useVision: "fallback" // Optional: use vision if text-based fails
});
// Extract data
...
The Stagehand class implementation can be found here:
...
The act() method uses this system prompt:

```typescript:lib/inference.ts
startLine: 87
endLine: 161
```
