...
The method allows for passing in either a valid JSON schema (like what is returned from [`zodToJsonSchema`](https://www.npmjs.com/package/zod-to-json-schema)).

Using the method is simple, just define your LLM and call `.withStructuredOutput({ ... })` with the desired schema.
...
Additionally, you can pass in a JSON schema directly:
...
If you need additional information like logprobs or token usage using a callback or by calling the lower level `.generate()` method.
