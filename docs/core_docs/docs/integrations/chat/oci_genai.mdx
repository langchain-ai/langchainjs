...
This notebook explains how to use OCI’s Genrative AI models with
LangChainJS.

## Prerequisites

In order to use this integration you will need the following: 1. An OCI
tenancy. If you do not already have and account, please create one
[here](https://signup.oraclecloud.com/). 2. Setup an [authentication
...
regions](https://docs.oracle.com/en-us/iaas/Content/generative-ai/overview.htm#regions).
4. You will need the ID (aka OCID) of a compartment in which your OCI
...
own](https://docs.oracle.com/en-us/iaas/Content/Identity/compartments/To_create_a_compartment.htm).
5. Retrieve the desired model name from the [available
...
correct information. Please see the prerequisites for more information.
2. The retry strategy will be set to a single attempt. If the first API
...
[configuration](https://docs.oracle.com/en-us/iaas/Content/Identity/Tasks/callingservicesfrominstances.htm).
2. Using the Sao Paulo region. 3. Up to 3 attempts will be made in case
API calls fail.
