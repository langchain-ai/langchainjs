...
:::note
The Google GenerativeAI API does not allow tool schemas to contain an object field with unknown properties.
The Google VertexAI package does support this pattern. See here for the [Google VertexAI package](/docs/integrations/chat/google_vertex_ai).
