...
```typescript
import { AzureCosmosDBVectorStore } from "@langchain/community/vectorstores/azure_cosmosdb_nosql";
```
...
See a [usage example](/docs/integrations/vectorstores/azure_cosmosdb_mongodb_vcore).

```typescript
import { AzureCosmosDBVectorStore } from "@langchain/community/vectorstores/azure_cosmosdb_mongodb_vcore";
```
