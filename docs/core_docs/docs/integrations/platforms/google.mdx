...
The [PostgresVectorStore](docs/integrations/vectorstores/google_cloudsql_pg) from the
@langchain/google-cloud-sql-pg provides a way to use the CloudSQL for PostgresSQL to store
...
`@langchain/google-cloud-sql-pg` provides a way to use the CloudSQL for PostgresSQL to store messages and provide conversation history.
...
```typescript
const historyInstance = await PostgresChatMessageHistory.create(
  engine,
  "test",
  "chat_message_table"
);
