...
The `docs` field also must contain the `sessionId` as this is the key used for identifying the chat session.
...
```
/chats/{user-id}/bots/{bot-id}/messages/{message-id}
```

import NestedExample from "@examples/memory/firestore_nested.ts";

<CodeBlock language="typescript">{ExaNestedExamplemple}</CodeBlock>
