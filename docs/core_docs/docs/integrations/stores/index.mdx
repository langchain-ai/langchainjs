...
- [`InMemoryStore`](/docs/integrations/storage/in_memory)
- [`LocalFileStore`](/docs/integrations/storage/file_system) (Node.js only)
...
For example the [`LocalFileStore`](/docs/integrations/storage/file_system) allows for persisting data through the file system. It also is incredibly fast, so your users will be able to access cached data in a snap.
