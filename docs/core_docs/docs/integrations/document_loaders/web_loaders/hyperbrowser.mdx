...
| `sessionOptions` | `object` | See below | Browser session configuration |

#### Session Options

```typescript
interface SessionOptions {
  useProxy?: boolean;      // Use a proxy for access
  solveCaptchas?: boolean; // Auto-solve CAPTCHAs
  acceptCookies?: boolean; // Handle cookie popups
  useStealth?: boolean;    // Enable stealth mode
}
```
