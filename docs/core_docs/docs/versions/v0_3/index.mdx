...
- Callbacks are now non-blocking by default rather than the reverse. This means that if you are using e.g. LangSmith for tracing in a serverless environment, you will need to [await the callbacks to ensure they finish before your function ends](/docs/how_to/callbacks_serverless).
...
- Made [LangGraph.js](https://js.langchain.com/docs/how_to/migrate_agent) the recommended way to create agents.
...
| @langchain/textsplitters | 0.1.0  | >=0.3.0 <0.4.0                        |
...
We also suggest checking your lockfile to make sure that your package manager only has one version of `@langchain/core` installed.
