...
The first step in a SQL chain or agent is to take the user input and convert it to a SQL query. LangChain comes with a built-in chain for this: [`createSqlQueryChain`](https://v02.api.js.langchain.com/functions/langchain.chains_sql_db.createSqlQueryChain.html)
...
We can use the [`QuerySqlTool`](https://v02.api.js.langchain.com/classes/langchain.tools_sql.QuerySqlTool.html) to easily add query execution to our chain:
...
- To initialize the agent, we use [`createOpenAIToolsAgent`](https://v02.api.js.langchain.com/functions/langchain.agents.createOpenAIToolsAgent.html) function.
  This agent contains the [`SqlToolkit`](https://v02.api.js.langchain.com/classes/langchain.agents_toolkits_sql.SqlToolkit.html) which contains tools to:
