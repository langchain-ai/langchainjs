...
export class CustomChatModel extends SimpleChatModel {
  ...
  async _call(
    ...
    // Can pass `runManager?.getChild()` when invoking internal runnables for tracing
...
If you want to take advantage of LangChain's callback system for functionality like token tracking, you can extend the `BaseChatModel` class and implement the lower level
