...
To create your own retriever, you need to implement a `_getRelevantDocuments` method that takes a `string` as its first parameter and an optional `runManager` for tracing.
...
export class CustomRetriever extends BaseRetriever {
  ...
  async _getRelevantDocuments(
    ...
    // You could invoke other runnables like this.
    // Note how we pass `runManager?.getChild()` to enable tracing:
