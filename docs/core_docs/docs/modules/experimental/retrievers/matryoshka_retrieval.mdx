...
The `MatryoshkaRetrieval` retriever takes in two vector stores: `smallStore` and `largeStore`.
...
Then, when `getRelevantDocuments` is called, the `MatryoshkaRetrieval` retriever will first perform
...
import Example from "@examples/retrievers/matryoshka_retrieval.ts";
