  ...
  cells: [
    {
      ...
      outputs: [
        {
          data: {
            "text/plain": ["[Module: null prototype] {  }"],
          },
          execution_count: 1,
          metadata: {},
...
function getPathToNotebookInitUtils(invokedFromDirectory) {
  ...
  if (coreDocsIndex === -1 || coreDocsIndex === pathParts.length - 1) {
    throw new Error(
      '"core_docs" must be part of the path and not the last element.'
    );
  }
...
function main() {
  ...
  const pathToNotebookInitUtils =
    getPathToNotebookInitUtils(invokedFromDirectory);
  const cellSourceContents = [
  ...
  fs.writeFileSync(
    pathToNewNotebook,
    JSON.stringify(TEMPLATE_NOTEBOOK, null, 2)
  );
}
