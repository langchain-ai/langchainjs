#!/usr/bin/env bash
#
# Zod compatibility tests for @langchain/core
#
# Tests that @langchain/core's exported types work correctly with:
#   1. zod@3.25.x (v3-only consumer)
#   2. zod@4.x (v4 consumer)
#   3. zod version mismatch (consumer has zod@3.25.x, core built with zod@4.x)
#
# Each test creates an isolated directory, installs @langchain/core from a
# local tarball alongside a specific zod version, and runs `tsc --noEmit`.
#
# Usage:
...
echo "=== Building @langchain/core ==="
cd "$CORE_DIR"
pnpm build 2>&1 | tail -3

echo "=== Packing @langchain/core ==="
TARBALL=$(pnpm pack --pack-destination "$WORK_DIR" 2>/dev/null | tail -1)
if [ ! -f "$TARBALL" ]; then
...
for test_name in "${TESTS[@]}"; do
  ...
  npm install --no-package-lock "$TARBALL" 2>&1 | tail -3
  ...
  echo "Running tsc --noEmit (timeout 60s, max 512MB heap)..."
  if timeout 60 node --max-old-space-size=512 ./node_modules/.bin/tsc --noEmit 2>&1; then
    echo "PASS: $test_name"
    PASS=$((PASS + 1))
  else
    echo "FAIL: $test_name"
