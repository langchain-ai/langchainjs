...
const simpleNativePrompt = PromptTemplate.fromTemplate<SimpleInput>(`{foo}{bar}`);
const simpleLodashPrompt = LodashPromptTemplate.fromTemplate<SimpleInput>(`{foo}{bar}`);
const simpleHandlebarsPrompt = HandlebarsPromptTemplate.fromTemplate<SimpleInput>(`{{foo}}{{bar}}`);

type ComplexInput = Record<string, string>;
const complexInput: Record<string, string> = Object.fromEntries([...Array(100).keys()].map((i) => [`key${i}`, `value${i}`]));

const complexNativePrompt = PromptTemplate.fromTemplate<ComplexInput>(Object.keys(complexInput)
	.map((key) => `${key}: {${key}}`)
	.join(`\n`));
const complexLodashPrompt = LodashPromptTemplate.fromTemplate<ComplexInput>(Object.keys(complexInput)
	.map((key) => `${key}: {${key}}`)
	.join(`\n`));
const complexHandlebarsPrompt = HandlebarsPromptTemplate.fromTemplate<ComplexInput>(Object.keys(complexInput)
	.map((key) => `${key}: {{${key}}}`)
	.join(`\n`));
...
if (formattedNative !== formattedLodash || formattedNative !== formattedHandlebars) {
