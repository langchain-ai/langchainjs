...
export const run = async () => {
    const vectorStore = await SingleStoreVectorStore.fromTexts(
        ...
        [{"category": "rain"}, {"category": "rain"}, {"category": "rain"}, {"category": "snow"}, {"category": "snow"}, {"category": "snow"}],
    ...
    const resultOne = await vectorStore.similaritySearch("rainstorm in parched desert, rain", 1, { "category": "rain" });
    console.log(resultOne[0].pageContent);

    await vectorStore.setSearchConfig({
        searchStrategy: "TEXT_ONLY",
    });
    const resultTwo = await vectorStore.similaritySearch("rainstorm in parched desert, rain", 1);
    ...
    const resultFive = await vectorStore.similaritySearch("rainstorm in parched desert, rain", 1);
