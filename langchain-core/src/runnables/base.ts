...
type FromFunc<T extends ZodAny = ZodAny> =
  | ((input: z.infer<T>, config?: RunnableConfig) => Promise<string>)
  | ((input: z.infer<T>, config?: RunnableConfig) => string);
...
> {
  ...
  static fromFunction<T extends ZodAny = ZodAny>(
    func: FromFunc<T>,
    fields: {
      name?: string;
      description?: string;
  ...
  ): StructuredTool<T> | Tool {
    const name = fields.name ?? func.name;
    const description =
      fields.description ??
      `Takes ${JSON.stringify(zodToJsonSchema(fields.schema), null, 2)}`;
    const jsonSchema = zodToJsonSchema(fields.schema);
    if (
      "type" in jsonSchema &&
      ...
      // Object input
      return new (class ToolCls extends StructuredTool<T> {
      ...
      return new (class ToolCls extends StructuredTool {
        schema = z
 ...
 */
export interface ToolWrapperParams<RunInput extends ZodAny = ZodAny>
  extends ToolParams {
  /**
